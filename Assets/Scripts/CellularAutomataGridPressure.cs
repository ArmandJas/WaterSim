...
public class CellularAutomataGridPressure : MonoBehaviour
{
    ...
    public int PressureAmount = 10; // amount of pressure (extra ml) that 1 voxel gives
    public int PressureImpactDampen = 10; // how much pressure impact on speed needs to be reduced
    ...
    public enum Shape
    {
        ...
        WalledBox,  // Box split in half with walls,
                    // one half full of liquid, wall has a 2 voxel gap
            ...
            for (int y = 0; y < YSize; y++)
            {
                for(int z = 0; z < ZSize; z++)
                {
                    ...
                    else if (ContainerShape == Shape.FillingBox)
                    {
                        ...
                        else if (x == XSize / 2 && y == YSize - 1 && z == ZSize / 2)
                        {
    ...
    private void checkVoxel(int x, int y, int z)
    {
        ...
        if (y != YSize - 1) // check pressure above
        {
            CellularAutomataLiquid higherNeighbor = voxelGrid[x, y + 1, z].GetComponent<CellularAutomataLiquid>();
            if (higherNeighbor.isSolid != true && curVoxel.volume > 1000 && higherNeighbor.volume < curVoxel.volume)
            { // flow up if pressurized
                maxFlow = FlowSpeed /* (curVoxel.volume - 999)*/;
                flow = Mathf.Min(((curVoxel.volume - higherNeighbor.volume) - PressureAmount) / 2, maxFlow, curVoxel.volume);

                higherNeighbor.volume += flow;
                curVoxel.volume -= flow;

                higherNeighbor.UpdateVolume();
        ...
        if (y != 0) // flow down
        {
            ...
            maxFlow = Mathf.Max(1000, curVoxel.volume + PressureAmount);
            if (lowerNeighbor.isSolid != true &&
                lowerNeighbor.volume < maxFlow)
            {
                // flows downward at any speed, adding pressure
                flow = Mathf.Min(maxFlow - lowerNeighbor.volume, curVoxel.volume);
        ...
        {
            if (destination.volume < curVoxel.volume)
            {
                maxFlow = FlowSpeed;
                if(curVoxel.volume > 1000)
                {
                    maxFlow = FlowSpeed * (curVoxel.volume - 1000) / PressureImpactDampen;
                }
